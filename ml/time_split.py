"""
Chronological splits for fight-level training (avoid future leakage).

Prefer ``game_date`` / ``event_date`` from logs; fall back to ``logged_at``.
"""
...
def resolve_sort_date(df: pd.DataFrame) -> tuple[pd.Series, str]:
    if "event_date" in df.columns and df["event_date"].notna().any():
        s = pd.to_datetime(df["event_date"], errors="coerce")
