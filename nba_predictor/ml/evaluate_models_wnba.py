"""
Evaluate logged WNBA home-win probabilities on merged rows; optional sklearn replay.

Run from ``nba_predictor``: ``python -m ml.evaluate_models_wnba``
"""
...
from utils.data_io import PATH_MERGED_WNBA, ROOT, ensure_dirs, load_registry, save_eval_report
...
def main() -> None:
    ensure_dirs()
    if not PATH_MERGED_WNBA.exists():
        print("No merged WNBA data.")
        return
    df = pd.read_parquet(PATH_MERGED_WNBA)
    report: dict[str, Any] = {"league": "wnba", "n_rows": int(len(df))}
    ...
    reg = load_registry()
    win_path = (reg.get("production") or {}).get("win_model_path_wnba")
    ...
    save_eval_report(report, league="wnba")
